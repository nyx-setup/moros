...
/* ASCII 8x8 bitmap font, starting at space (0x20) */
const uint8_t moros_font8x8[96][8] = {
...
void moros_draw_char(uint16_t x, uint16_t y, char c, uint16_t color, uint8_t scale)
{
    ...
    const uint8_t *bitmap = moros_font8x8[c - 0x20];

    for (int row = 0; row < 8; row++) {
        for (int col = 0; col < 8; col++) {
            if (bitmap[row] & (0x01 << col)) {
                for (int sy = 0; sy < scale; sy++)
                    for (int sx = 0; sx < scale; sx++)
                        moros_display_set_pixel(
                            x + col * scale + sx,
                            y + row * scale + sy,
                            color
                        );
...
{
    while (*text) {
        moros_draw_char(x, y, *text, color, scale);
        x += 8 * scale;
        text++;
    }
}
